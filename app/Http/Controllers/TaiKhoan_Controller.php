...
class TaiKhoan_Controller extends Controller
{
    ...
    public function postDangNhap(Request $req){
        ...
        if(Auth::attempt($credentials)){
            ...
            if (Auth::user()->level == 1 || Auth::user()->level == 3 || Auth::user()->level == 4) {
                ...
                Session::put('user_email_login', Auth::user()->email);
    ...
    public function userUpdate1(Request $req)
    {
        if (Auth::check()) {
            ...
            return redirect()->back()->with('thongbaoloi', 'Cập nhật thất bại!');  
    ...
    public function passwordUpdate(Request $req)
    {
        if (Auth::check()) {
            ...
            $userUpdate =  User::find(Auth::user()->id );
            if (Hash::check($req->old_password, $userUpdate->password))
            {
            $userUpdate->password = Hash::make($req->password);
            $userUpdate->save();
            ...
            return redirect()->back()->with('thongbaoloi', 'Mật khẩu không đúng. Vui lòng kiểm tra lại!');  
        }
    }
}

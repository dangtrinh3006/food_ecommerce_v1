...
class admincontroller extends Controller
{
    ...
    public function getIndexAdminDash(Request $req){
    	if ((Auth::check() && Auth::user()->level == 1) || (Auth::check() && Auth::user()->level == 3) || (Auth::check() && Auth::user()->level == 4) ){
...
/*-----------------------------------------------User--------------------------------------------------------------------*/
    public function getQL_NguoiDung(Request $req){
        ...
        if (Auth::check() && Auth::user()->level == 1)  {
            ...
            return redirect()->back()->with('thongbaoloi', 'Bạn không đủ quyền!'); 
        }
        
    }

    public function getQL_NguoiDung_user(Request $req){
        $resuft_tb_1 = trans('home.Notification_permision', [], 'vi');
    ...
    public function getQL_NguoiDung_ad(Request $req){
        ...
        if (Auth::check() && Auth::user()->level == 1) {
            $taikhoan_ad = User::where('level','!=',2)->get();
...
/*-----------------------------------------------Slide-------------------------------------------------------------------*/
    ...
    public function postUpdateSlide(Request $req, $id){
        if ( (Auth::check() && Auth::user()->level == 1) || (Auth::check() && Auth::user()->level == 3) ) {
            // dd($id);
...
/*-----------------------------------------------NSX--------------------------------------------------------------------*/
    public function getQL_Nsx(Request $req){
        if ( (Auth::check() && Auth::user()->level == 1) || (Auth::check() && Auth::user()->level == 4)) {
...
/*-----------------------------------------------Đơn-Hàng----------------------------------------------------------------*/
    ...
    public function getDonHang_daduyet(Request $req)
    {
        if ( (Auth::check() && Auth::user()->level == 1) || (Auth::check() && Auth::user()->level == 3) || (Auth::check() && Auth::user()->level == 4)) {
            ...
            return redirect()->router('trang-chu-admin')->with('error',''.$resuft_tb_1.'');   
        }
    }
    ...
    public function print_order_convert($checkout_code){
        ...
        <div style="float: left; margin: 0 0 1.5em 0; ">
         <strong style="font-size: 18px;">GOOD FOOD</strong>
          <br />
          <strong>Địa chỉ:</strong> 10, Nguyễn Hữu Thọ, phường Tân Phong, Quận 7, Thành phố Hồ Chí Minh.
          <br/>
          <strong>Điện thoại:</strong> 0834 021123
          <br/>
          <strong>Website:</strong> goodfood.com
          <br/>
          <strong>Email:</strong> order.goodfood2021@gmail.com
            ...
            foreach($billdetaill_print as $key => $bd){
                foreach($bill_print as $key2 => $b_print){
                    if ($kh_print->payment == 'ATM') {
                        ...
                        $toto = number_format($bd->unit_price,0,',','.') * $bd->quantity + 30;
                    }
                    $output.='
                    <tr valign="top" style="border-top: 1px solid #d9d9d9;">
                      ...
                      <td align="right" style="padding: 5px 0px">'.number_format($bd->unit_price,0,',','.').' VNĐ</td>
                    </tr>';
                }
            }
            $output.='
        ...
        <table style="font-size: 12px;width: 100%; margin: 0 0 1.5em 0;">
          <tr>
            ...
            <td style="text-align:right">'.number_format($b_print->total,0,',','.').'VNĐ</td>
          </tr>
          <tr>
              <td style="width: 50%;padding: 5px 0px">Phí vận chuyển:</td>
              <td style="text-align:right;padding: 5px 0px">30.000 VNĐ</td>
            </tr>
          <tr>
            <td style="padding: 5px 0px"><strong>Tổng tiền:</strong></td>
            <td style="text-align:right;padding: 5px 0px"><strong><p>'.$toto.'.000 VNĐ</td>
          </tr>
        </table>
        ...
        </td></tr></table><br/><br/><br/><p>Chúc quý khách ngon miệng! || Email: <u>order.goodfood2021@gmail.com</u> || Hotline: 0834021123</p></div>
        ';
...
/*-----------------------------------------------Lang---------------------------------------------------------------*/
    ...
    public function send_coupon(){
        ...
        $to_email =  "order.goodfood2021@gmail.com";
        $title_mail = "MÃ KHUYẾN MÃI TỪ GOOD FOOD".' '.$now_send;
